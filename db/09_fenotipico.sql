...
CREATE TABLE IF NOT EXISTS fenotipico.evaluacion_fenotipica (
    ...
    variedad_id      INTEGER NOT NULL REFERENCES agro.variedad(id) ON DELETE RESTRICT ON UPDATE CASCADE,
    ciclo_eval       VARCHAR(20) NOT NULL,        -- 'OI-2024', 'PV-2024'
    anio_eval        SMALLINT NOT NULL,
    comunidad_id     UUID REFERENCES core.comunidad(id) ON DELETE SET NULL ON UPDATE CASCADE, -- localidad normalizada
    evaluador_id     UUID REFERENCES sistema.usuario(id) ON DELETE SET NULL ON UPDATE CASCADE,      -- evaluador normalizado
...
CREATE INDEX IF NOT EXISTS idx_eval_variedad ON fenotipico.evaluacion_fenotipica(variedad_id);
...
CREATE INDEX ix_ef_ciclo     ON fenotipico.evaluacion_fenotipica(ciclo_eval, anio_eval);
...
CREATE TABLE IF NOT EXISTS fenotipico.valores_fenotipicos (
    ...
    descriptor_id    SMALLINT        NOT NULL REFERENCES catalogo.descriptores(id),
...
CREATE TABLE IF NOT EXISTS fenotipico.muestra_nutrimental (
    ...
    germoplasma_id      UUID NOT NULL REFERENCES core.germoplasma(id) ON DELETE RESTRICT ON UPDATE CASCADE,   -- FK a agronomico.germoplasma
    parcela_id          UUID NOT NULL REFERENCES core.parcela(id) ON DELETE RESTRICT ON UPDATE CASCADE,   -- FK a geografico.parcela
    comunidad_id        UUID REFERENCES core.comunidad(id) ON DELETE SET NULL,
